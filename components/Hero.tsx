...
export default function Hero({ onInitialImageLoad }: HeroProps) {
  const [current, setCurrent] = useState(0);
  const [isInitialImageLoaded, setIsInitialImageLoaded] = useState(false);
  const hasSignaledInitialLoad = useRef(false);
  ...
  const markInitialImageAsLoaded = useCallback(() => {
    if (hasSignaledInitialLoad.current) return;
    hasSignaledInitialLoad.current = true;
  ...
  }, [isInitialImageLoaded, startTimer]);
      ...
      <AnimatePresence initial={false}>
        ...
        >
          <Image
            ...
            priority={current === 0}
            sizes="100vw"
            onLoadingComplete={current === 0 ? markInitialImageAsLoaded : undefined}
            onError={current === 0 ? markInitialImageAsLoaded : undefined}
          />
        </motion.div>
      </AnimatePresence>
        ...
        <div className="mx-auto max-w-4xl text-center">
          ...
          */}
          ...
          >
            ...
            désherbage : nous gérons tout pour vous, sans aucune avance de trésorerie.
          </motion.p>
